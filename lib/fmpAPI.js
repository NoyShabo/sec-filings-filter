...
import { findOTCTickerByName } from './otcMarketsCache.js'
import { rateLimitedFMPRequest } from './fmpRateLimiter.js'
...
export async function fetchMarketCapByTicker(ticker) {
  ...
  try {
    const response = await rateLimitedFMPRequest(() => 
      axios.get(`${FMP_BASE_URL}/market-capitalization/${ticker}`, {
...
export async function fetchMarketCapBatch(tickers) {
  ...
  // Process all tickers - rate limiter will handle the delays automatically
...
export async function fetchCompanyProfile(ticker) {
  ...
  try {
    const response = await rateLimitedFMPRequest(() =>
      axios.get(`${FMP_BASE_URL}/profile/${ticker}`, {
        params: { apikey: FMP_API_KEY },
        timeout: 10000,
      })
    );
...
export async function cikToTicker(cik, companyName = null) {
  ...
  try {
    ...
    const cikResponse = await rateLimitedFMPRequest(() =>
      axios.get(`${FMP_BASE_URL}/cik-search/${paddedCik}`, {
        params: { apikey: FMP_API_KEY },
        timeout: 10000,
      })
    );
    ...
    if (companyName) {
      ...
      if (cleanName.length >= 3) {
        ...
        const nameResponse = await rateLimitedFMPRequest(() =>
          axios.get(`${FMP_BASE_URL}/search`, {
            params: { 
              query: cleanName,
              limit: 1,
    ...
    // Method 5: Fallback to OTC Markets API (for OTC/penny stocks)
    if (companyName) {
      console.log(`Searching OTC Markets for: "${companyName}"`);
      const otcTicker = await findOTCTickerByName(companyName);
      if (otcTicker) {
        return otcTicker;
      }
    }
...
export async function fetchSECFilingsFromFMP({ fileType, startDate, endDate, page = 0, limit = 100 }) {
  ...
  try {
    ...
    const response = await rateLimitedFMPRequest(() =>
      axios.get(`${FMP_V4_BASE_URL}/rss_feed`, {
        params: {
          type: fileType,
          from: startDate,
          to: endDate,
