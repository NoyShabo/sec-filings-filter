  ...
  if (fmpAvailable) {
    try {
      ...
      if (fetchAll) {
        ...
        const MAX_FMP_PAGES = 100; // Safety limit

        console.log(`[FMP] Starting to fetch all pages...`);

        while (hasMore && consecutiveEmptyPages < 2 && currentPage < MAX_FMP_PAGES) {
          console.log(`[FMP] Fetching page ${currentPage}...`);
          ...
          console.log(`[FMP] Page ${currentPage}: Got ${filings.length} filings`);

          if (filings.length === 0) {
            consecutiveEmptyPages++;
            console.log(`[FMP] Empty page (${consecutiveEmptyPages}/2)`);
          } else {
            consecutiveEmptyPages = 0;
            allFilings.push(...filings);
          }
          
          // Stop if we got less than 100 results (last page)
          if (filings.length < 100) {
            console.log(`[FMP] Last page detected (${filings.length} < 100)`);
            hasMore = false;
          } else {
            currentPage++;
          }
        ...
        if (currentPage >= MAX_FMP_PAGES) {
          console.warn(`⚠️ Reached FMP page limit (${MAX_FMP_PAGES} pages). There may be more results.`);
        }

        console.log(`✓ FMP pagination complete: ${allFilings.length} total filings from ${currentPage + 1} pages`);
        return allFilings;
      } else {
