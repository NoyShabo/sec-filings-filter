  ...
  while (hasMoreResults && pageCount < MAX_PAGES) {
    try {
      ...
      for (const filing of filings.filings) {
        // Create unique ID using multiple fields to catch all duplicates
        const filingId = `${filing.cik}-${filing.formType}-${filing.filingDate}-${filing.fileNumber || 'none'}-${filing.company}`;
        if (!seenFilings.has(filingId)) {
          seenFilings.add(filingId);
          allFilings.push(filing);
          newFilingsCount++;
        } else {
          console.log(`[DUPLICATE] Skipping: ${filing.company} (${filing.cik}) - ${filing.formType} on ${filing.filingDate}`);
        }
      }
...
function parseAtomResponse(data) {
  ...
  for (const entry of entries) {
    try {
      ...
      if (!company || !cik || !formType) {
        // Extract from title: "10-K - ATMOS ENERGY CORP (0000731802) (Filer)"
        // or "NT 10-K - Company Name (1234567) (Filer)"
        const titleMatch = title.match(/^(.+?)\s*-\s*(.+?)\s*\((\d+)\)/);
        if (titleMatch) {
          if (!formType) formType = titleMatch[1].trim();
          if (!company) company = titleMatch[2].trim();
          if (!cik) cik = titleMatch[3].trim();
        }
      }
      
      // Clean up company name if it still contains the form type prefix
      ...
      if (filing.company && filing.cik && filing.formType) {
        // Final check: don't use title as company name
        if (filing.company === title) {
          console.warn(`⚠️ Skipping filing - company name equals title: ${title}`);
          continue;
        }
        filings.push(filing);
      } else {
        console.warn(`⚠️ Skipping filing - missing data: company=${company}, cik=${cik}, formType=${formType}`);
      }
    } catch (error) {
