...
export async function processFilings(filings, filters) {
  ...
  const tickerPromises = uniqueCiks.map(async (cik) => {
    try {
      ...
      console.error(`Error converting CIK ${cik}:`, error.message);
    }
  });
  ...
  if (tickers.length > 0) {
    console.log(`Fetching market cap for ${tickers.length} tickers...`);
    // Fetch company profiles which include market cap
    const profilePromises = tickers.map(async (ticker) => {
      try {
        const profile = await fetchCompanyProfile(ticker);
        if (profile && profile.marketCap) {
          marketCapData[ticker] = profile;
          console.log(`✓ ${ticker} → Market Cap: $${(profile.marketCap / 1e9).toFixed(2)}B`);
        } else {
